...
export const removeLiquidityV3 = async (account: PrivateKeyAccount, tokenId: BigInt, percent: number) => {
    ...
    const involvesETH =
        getAddress(bnb.wrapped.address) === getAddress(positionInfo[2]) ||
    ...
    if (involvesETH) {
        const poolAddrs = await client.readContract({
            ...
            args: [positionInfo[2] as Address, positionInfo[3] as Address, positionInfo[4]]
        });
        ...
        const ethAmount =
            getAddress(bnb.wrapped.address) === getAddress(positionInfo[2]) ? discountedAmount0 : discountedAmount1;
        const token =
            getAddress(bnb.wrapped.address) === getAddress(positionInfo[2]) ? positionInfo[3] : positionInfo[2];
        const tokenAmount =
            getAddress(bnb.wrapped.address) === getAddress(positionInfo[2]) ? discountedAmount1 : discountedAmount0;
