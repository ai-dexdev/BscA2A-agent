// @ts-nocheck
import { Hash, getContract, Hex, parseUnits, isAddress, PrivateKeyAccount, Address } from "viem";
...
export const getToken = async (token: string) => {
    ...
    return new Token(ChainId.BSC, address as Hex, decimal, symbol);
};
...
export const pancakeSwap = async ({
    ...
    if (!currencyA.isNative) {
        ...
        if (!TokenContract.write || !TokenContract.write.approve || !TokenContract.read.allowance) {
