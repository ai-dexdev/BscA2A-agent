// @ts-nocheck
import { parseAbi, type Address, Hex, PrivateKeyAccount } from "viem";
import { Pool, Position, nearestUsableTick, FeeAmount, encodeSqrtRatioX96 } from "@pancakeswap/v3-sdk";
...
async function checkBalance(account: PrivateKeyAccount, token: Currency, amount: CurrencyAmount<Currency>) {
    const accountAddress = account.address;
    if (token.isNative) {
...
export async function addLiquidityV3(
    ...
    const value = tokenA.isNative ? amountA.quotient.toString() : tokenB.isNative ? amountB.quotient.toString() : "0";
